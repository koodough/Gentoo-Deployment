...
#Have messages for anything important than need to be compiled in the kernel
...
function function_encapsulator
{
...
function gentoo_commander
{
	...
	case "$when" in
		...
		"post_message")	POST_MESSAGE="$POST_MESSAGE $command";;
	esac
}
...
function pre_chroot
{
	...
	case $architecture in
		x86|i686)
			STAGE3_URL="http://mirrors.cs.wmich.edu/gentoo/releases/x86/current-stage3/stage3-i686-20110524.tar.bz2";;
		amd64)
			STAGE3_URL="http://mirrors.cs.wmich.edu/gentoo/releases/amd64/current-stage3/stage3-amd64-20110524.tar.bz2";;
		*)
...
###########################
	...
	pre_chroot && pre_install && save_variables;
fi
